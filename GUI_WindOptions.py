...
# The following code represents the functionality for Wind Options.
...
guiWindow_WindOptions_Width = 470
guiWindow_WindOptions_Height = 225

# Formula to acquire date from one year ago in YYYY format. NOAA's portal for
# downloading wind data may take a year or more to have latest data available.
...
# Tuple for populating wind timespan text values within the combo box.
textCombo_WindTimespan = ()
textCombo_WindTimespan = textCombo_WindTimespan + \
...
while int_YearIterator >= 2008:
    textCombo_WindTimespan = textCombo_WindTimespan + (str(int_YearIterator),)
...
textCombo_WindTimespan = textCombo_WindTimespan + ("2005-2007", "2000-2004",
                                                   "90-99", "80-89", "70-79",
                                                   "60-69", "55-59",
                                                   "Custom...")
...
# Tuple for populating wind speed text values within the combo box.
textCombo_WindSpeed = ("Select...", "All", "30 - 60", "60 - 90",
                          "90 - 120", "120+", "Custom...")
...
# Tuple for populating the speed combobox with integer values for the
# custom speed option.
intList_Speed = []
int_SpeedIterator = 30
while 200 >= int_SpeedIterator:
    intList_Speed.append(int_SpeedIterator)
    int_SpeedIterator = int_SpeedIterator + 10

int_Combo_Speeds = intList_Speed
...
# wind CSVs from NOAA website (used for non-custom parameter inputs only).
wind_siteURL = "http://www.spc.noaa.gov/wcm/data/"

# Comma-delimited header names for the NOAA wind CSVs.
wind_headers = "om" + "," + "yr" + "," + "mo" + "," + "dy" + "," + "date" + \
               "," + "time" + "," + "tz" + "," + "st" + "," + "stf" + "," + \
               "stn" + "," + "mag" + "," + "inj" + "," + "fat" + "," + \
...
# created by the user.
urlDialog_Message = "URL will populate once Timespan and Speed have been " \
                    "selected."
...
# Column header from CSV file for speed, used as an input parameter for the
...
try:
    ...
    # wind and polygon feature classes.
...
# Class for the Wind Options GUI functionality.
class WindOptions(FrameLifts):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.comboFrame_Custom_Speed = None
        self.custom_speed_url = None
        ...
        self.intCustom_Speed_Min = None
        self.intCustom_Speed_Max = None
        self.textCustomSpeedFrom = None
        self.textCustomSpeedTo = None
        self.custom_Speed_Naming = None
        ...
        # Settings and configuration for the Wind Options GUI window.
        self.winfo_toplevel().title("Wind Options")
        self.winfo_toplevel().geometry("%dx%d" %
                                       (guiWindow_WindOptions_Width,
                                        guiWindow_WindOptions_Height))
        ...
        # Frame that displays the initial Wind dialog layout.
        ...
        # Executes the function controlling the Timespan/Speed comboboxes
    ...
    def func_ComboFrame(self):
        ...
        # Widget frame housing the combobox items for Timespan/Speed.
        ...
        # Label for Speed combobox.
        comboLabel_Speed = ttk.Label(self.comboFrame,
                                        text="Speed (Knots):")
        comboLabel_Speed.grid(column=2, row=0, sticky=tkinter.W)

        # Combobox requirements for Timespan.
        self.stringWindTimespan = tkinter.StringVar()
        self.combo_WindTimespan = ttk.Combobox(self.comboFrame, width=13,
                                        textvariable=self.stringWindTimespan,
                                        state="readonly")
        self.combo_WindTimespan["values"] = (textCombo_WindTimespan)
        self.combo_WindTimespan.grid(column=0, row=1, sticky=tkinter.W)

        # GIF icon to symbolize Wind within the GUI.
        dirFolder = os.path.dirname(__file__)
        gifPath = os.path.join(dirFolder, "Icon_Wind.gif")
        self.photo_Wind_Icon = \
            tkinter.PhotoImage(file=gifPath)
        colorCode_Black = "black"
        self.subSampleImage = self.photo_Wind_Icon.subsample(4, 4)
        self.label_for_icon = ttk.Label(self.comboFrame, borderwidth=2,
                                        relief="solid",
                                        background=colorCode_Black,
                                        image=self.subSampleImage)
        self.label_for_icon.photo = self.photo_Wind_Icon
        self.label_for_icon.grid(column=1, row=0, padx=0,
                                 pady=5, sticky=tkinter.EW, rowspan=2)

        # Combobox requirements for Speed.
        self.stringWindSpeed = tkinter.StringVar()
        self.combo_WindSpeed = ttk.Combobox(self.comboFrame, width=12,
                                            textvariable=self.stringWindSpeed,
                                            state="readonly")
        self.combo_WindSpeed["values"] = (textCombo_WindSpeed)
        self.combo_WindSpeed.grid(column=2, row=1, sticky=tkinter.W)

        # Combobox selection binding that will be used to populate URL Field.
        self.combo_WindTimespan.bind("<<ComboboxSelected>>",
                                     self.func_PopulateURLField_Timespan)

        # Combobox selection binding that will be used to populate URL Field.
        self.combo_WindSpeed.bind("<<ComboboxSelected>>",
                                     self.func_PopulateURLField_Speed)
        ...
        self.combo_WindTimespan.current(0)

        # Display first index for Speed (Select...).
        self.combo_WindSpeed.current(0)
    ...
    def func_ComboCustomSpeed(self):

        # This function controls the display of the custom combobox frame
        # controlling the custom Speed.

        # If the custom speed combobox frame already exists (is visible)...
        if self.comboFrame_Custom_Speed is not None:
            # Remove the frame.
            self.comboFrame_Custom_Speed.grid_remove()

        # Widget frame for custom speed.
        self.comboFrame_Custom_Speed = tkinter.Frame(self.initialFrame)
        self.comboFrame_Custom_Speed.grid(column=0, row=1, padx=60, pady=5,
                                             sticky=tkinter.E)

        # Label frame for the speed range comboboxes.
        self.comboLabelFrame_Custom_Speed = ttk.LabelFrame(
            self.comboFrame_Custom_Speed, text="Range:",
            labelanchor=tkinter.N)
        self.comboLabelFrame_Custom_Speed.grid(column=0, row=0, padx=5,
                                                  pady=5, sticky=tkinter.E)

        # Label for MINIMUM speed.
        comboLabel_Custom_Speed_Min = ttk.Label(
            self.comboLabelFrame_Custom_Speed, text="Min:        ")
        comboLabel_Custom_Speed_Min.grid(column=0, row=0, sticky=tkinter.W)

        # Label for MAXIMUM speed.
        comboLabel_Custom_Speed_Max = ttk.Label(
            self.comboLabelFrame_Custom_Speed, text="Max:")
        comboLabel_Custom_Speed_Max.grid(column=3, row=0, sticky=tkinter.W)

        # Combobox requirements for MINIMUM speed.
        self.intCustom_Speed_Min = tkinter.IntVar()
        self.combo_Custom_Speed_Min = ttk.Combobox(
            self.comboLabelFrame_Custom_Speed, width=4,
            textvariable=self.intCustom_Speed_Min, state="readonly")
        self.combo_Custom_Speed_Min["values"] = (int_Combo_Speeds)
        self.combo_Custom_Speed_Min.grid(column=0, row=1, sticky=tkinter.W)

        # Combobox requirements for MAXIMUM speed.
        self.intCustom_Speed_Max = tkinter.IntVar()
        self.combo_Custom_Speed_Max = ttk.Combobox(
            self.comboLabelFrame_Custom_Speed, width=4,
            textvariable=self.intCustom_Speed_Max, state="readonly")
        self.combo_Custom_Speed_Max["values"] = (int_Combo_Speeds)
        self.combo_Custom_Speed_Max.grid(column=3, row=1, sticky=tkinter.E)

        # For all items in the custom speed comboFrame, configure their
        # grids the same way.
        for child in self.comboFrame_Custom_Speed.winfo_children():
            child.grid_configure(padx=10, pady=1)

        # Display first index for MINIMUM speed (0.0).
        self.combo_Custom_Speed_Min.current(0)

        # Display last index for MAXIMUM speed (10.0).
        self.combo_Custom_Speed_Max.current(17)

        # Combobox selection binding that will be used to populate URL field.
        self.combo_Custom_Speed_Min.bind("<<ComboboxSelected>>",
                                            self.func_Set_Custom_Speed)

        # Combobox selection binding that will be used to populate URL field.
        self.combo_Custom_Speed_Max.bind("<<ComboboxSelected>>",
                                            self.func_Set_Custom_Speed)
    ...
    def func_Set_Custom_Speed(self, event):

        # This function handles events related to the custom Speed options
        # that a user selects. Based on those selections, this function will
        # assign values and trigger other actions if necessary.

        try:

            # If the custom MINIMUM speed has a value...
            if str(self.intCustom_Speed_Min.get()):
                ...
                self.textCustomSpeedFrom = str(
                    self.intCustom_Speed_Min.get())

            # If the custom MAXIMUM speed has a value...
            if self.intCustom_Speed_Max.get():
                # Convert/assign the double value to string.
                self.textCustomSpeedTo = str(
                    self.intCustom_Speed_Max.get())

            # If both combobox parameters for custom speed exist...
            if self.textCustomSpeedFrom is not None and \
                    self.textCustomSpeedTo is not None:
                pass

            # The combined string with adjustments used for file naming and
            # analyses later on in the script.
            self.custom_Speed_Naming = \
                self.textCustomSpeedFrom.replace(".", "_") + "_to_" + \
                self.textCustomSpeedTo.replace(".", "_")
    ...
    def func_PopulateURLField_Timespan(self, event):
        ...
        try:
            ...
            if self.stringWindTimespan.get() == "Select...":
            ...
            # If the user selects all wind data from combobox...
            elif self.stringWindTimespan.get() == \
                    "All | 1955-" + str(int_OneYearAgo):

                # Assign/clear variables as needed.
                self.timespan_url = "1955-" + str(int_OneYearAgo) #+ "_wind"

                # Set the URL text of non custom timespan/speed values.
                self.text_Wind_URL = wind_siteURL + self.timespan_url + "_" + \
                                     wind + fileExtCSV + fileExtZip

                if self.stringWindSpeed.get() != "Select...":
                    self.textURLTextbox.set(self.text_Wind_URL)
            ...
            elif self.stringWindTimespan.get() == "Custom...":
            ...
            # If the user selects any option aside from Select, All, or Custom..
                ...
                # Assign/clear variables as needed.
                self.timespan_url = self.stringWindTimespan.get() #+ "_wind"

                # Set the URL text of timespan values.
                self.text_Wind_URL = \
                    wind_siteURL + self.timespan_url + "_" + wind + fileExtCSV
    ...
    def func_PopulateURLField_Speed(self, event):

        # This function handles the events where a user selects a speed
        # option from the combobox. The selection will trigger various responses
        # as detailed below.

        # If the speed combobox displays "Select..."...
        if self.stringWindSpeed.get() == "Select...":
            ...
            # If the custom speed comboboxes exist, remove them from GUI.
            if self.comboFrame_Custom_Speed is not None:

                self.comboFrame_Custom_Speed.grid_remove()

        # If the user selects "All" from the combobox...
        elif self.stringWindSpeed.get() == "All":
        ...
        # If the user selects 30 - 60 from the combobox...
        elif self.stringWindSpeed.get() == "30 - 60":

            # Assign/clear variables as needed.
            self.mag_naming = "30_to_60"
            self.custom_speed_url = None
        ...
        # If the user selects 60 - 90 from the combobox...
        elif self.stringWindSpeed.get() == "60 - 90":

            # Assign/clear variables as needed.
            self.mag_naming = "60_to_90"
            self.custom_speed_url = None
        ...
        # If the user selects 90 - 120 from the combobox...
        elif self.stringWindSpeed.get() == "90 - 120":

            # Assign/clear variables as needed.
            self.mag_naming = "90_to_120"
            self.custom_speed_url = None
        ...
        # If the user selects 120+ from the combobox...
        elif self.stringWindSpeed.get() == "120+":

            # Assign/clear variables as needed.
            self.mag_naming = "120_and_Above"
            self.custom_speed_url = None
        ...
        # If the user selects "Custom..." from the combobox...
        elif self.stringWindSpeed.get() == "Custom...":

            # Assign/clear variables as needed.
            # Custom wind speed will default to All, as all data is initially
            # downloaded.
            self.mag_naming = "all"
            ...
            # speed comboboxes.
            self.func_ComboCustomSpeed()

            # Run the function that sets the custom speed variables.
            self.func_Set_Custom_Speed(event)
    ...
    def func_WorkspaceFolderFrame(self):
        ...
        self.workspaceFolderTextbox_WindFolderDialog = \
            ttk.Entry(self.workspaceFolderFrame,
                      textvariable=self.textWorkspaceDialogPath, width=52,
                      state="readonly")
        self.workspaceFolderTextbox_WindFolderDialog.grid(column=1,
                                                          row=1, padx=0, pady=0,
                                                          sticky=tkinter.W)
    ...
    def func_OKClick(self):
        ...
        try:
            ...
            self.okClick_NonCustom_ExpansionHeight = \
                guiWindow_WindOptions_Height + 200

            # If a custom timespan or custom speed has been selected...
            if self.customSelection_Height is not None:
                # Expand the height of the GUI to accomodate the Custom
                # timespan/speed height, plus the processing messages and
                # progress bar.
            ...
            # If the user has selected a valid timespan, speed, and assigned
            # an output workspace folder...
            if self.stringWindTimespan.get() != "Select..." and \
                    self.stringWindSpeed.get() != "Select..." and \
                    self.workspaceFolderTextbox_WindFolderDialog.get() != \
                ...
                if self.statusVar_Checkbutton_Options.get() == 1:
                    ...
                    # If the timespan or speed combobox is "Custom..."...
                    if self.stringWindTimespan.get() == "Custom..." or \
                            self.stringWindSpeed.get() == "Custom...":
                        ...
                        # If the custom minimum speed and the custom
                        # maximum speed comboboxes have values...
                        if self.intCustom_Speed_Min is not None and \
                                self.intCustom_Speed_Max is not None:

                            # If the minimum speed exceeds the maximum
                            # speed...
                            if self.intCustom_Speed_Min.get() > \
                                    self.intCustom_Speed_Max.get():
                                # Display error message.
                                messagebox.showerror(errorMessage_Header,
                                        message="Invalid speed entry.\n" +
                ...
                # Else if the "More Options" checkbox is not selected...
                    ...
                    # If the timespan or speed "Custom..." comboboxes are
                    # selected...
                    if self.stringWindTimespan.get() == "Custom..." or \
                            self.stringWindSpeed.get() == "Custom...":
                        ...
                        # If both the minimum and maximum speed comboboxes
                        # have values...
                        if self.intCustom_Speed_Min is not None and \
                                self.intCustom_Speed_Max is not None:
                ...
                # Else if the speed combobox shows "Select..."...
                elif self.stringWindSpeed.get() == "Select...":
    ...
    def func_CheckboxOptions(self):
        ...
        if self.statusVar_Checkbutton_Options.get() == 1:
            ...
            # Remove the More Options frame with Analysis Options and Clipping.
    ...
    def func_AnalysisOptionsFrame(self):
        ...
        self.checkbox_IDW = tkinter.Checkbutton(self.analysisOptionsLabelFrame,
                                            text="IDW",
                                            variable=self.statusAnalysis_IDW)
        # , command=self.test_IDW_parameters)
        self.checkbox_IDW.grid(column=0, row=0, padx=0, pady=0,
                               sticky=tkinter.W)
        ...
        self.checkbox_NaturalNeighbor = \
            tkinter.Checkbutton(self.analysisOptionsLabelFrame,
                                text="Nat. Neigh.",
                                variable=self.statusAnalysis_NaturalNeighbor)
        # command=self.func_Controls_For_Analysis_Options)
        ...
        self.checkbox_OptHotSpot = \
            tkinter.Checkbutton(self.analysisOptionsLabelFrame,
                                text="Opt. Hot Spot",
                                variable=self.statusAnalysis_OptHotSpot)
        # command=self.func_Controls_For_Analysis_Options)
        ...
        self.checkbox_PointDensity = \
            tkinter.Checkbutton(self.analysisOptionsLabelFrame,
                                text="Point Dens.",
                                variable=self.statusAnalysis_PointDensity)
    ...
    def func_CreateFolder(self):
        ...
        try:
            ...
            if self.statusVar_Checkbutton_Options.get() == 1:
                ...
                # If both timespan/speed comboboxes are not "Custom..."...
                if self.stringWindTimespan.get() != "Custom..." and \
                        self.stringWindSpeed.get() != "Custom...":
                ...
                # If timespan combobox is "Custom..." and speed combobox is
                # not "Custom..."...
                if self.stringWindTimespan.get() == "Custom..." and \
                        self.stringWindSpeed.get() != "Custom...":
                ...
                # If timespan is "Custom..." and speed is "Custom..."...
                if self.stringWindTimespan.get() == "Custom..." and \
                        self.stringWindSpeed.get() == "Custom...":
                ...
                # If timespan is not "Custom..." and speed is "Custom..."...
                if self.stringWindTimespan.get() != "Custom..." and \
                        self.stringWindSpeed.get() == "Custom...":
            ...
            if self.statusVar_Checkbutton_Options.get() == 0:
                ...
                # If timespan combobox is not "Custom..." and speed combobox
                # is "Custom..."...
                if self.stringWindTimespan.get() != "Custom..." and \
                        self.stringWindSpeed.get() == "Custom...":
    ...
    def func_DownloadCSV(self):
        ...
        try:
            ...
            if self.stringWindTimespan.get() != "Custom...":

                # If timespan equals All...
                if self.stringWindTimespan.get() == \
                        "All | 1955-" + str(int_OneYearAgo):
                    ...
                    os.rename(self.csvDirectory + self.timespan_url + "_" +
                              wind +
                              fileExtCSV, self.csvDirectory + noaa + "_" +
                              wind + "_" +
                              str(self.timespan_url).replace("-", "_to_") +
                              curDate + fileExtCSV)
            ...
            if self.stringWindTimespan.get() == "Custom...":
                ...
                # Rename the unzipped file to better represent the input
                # user-defined parameters.
                os.rename(self.csvDirectory + self.timespan_url + "_" + wind +
                          fileExtCSV, self.csvDirectory + noaa + "_" + wind +
    ...
    def func_NonCustomTimespan_CSV_HeaderCheck_ValueCheck(self):
        ...
        try:
            ...
            if self.stringWindSpeed.get() == "Custom...":
                ...
                self.nameFeatureClass_FromCSV = \
                    noaa + "_" + wind + "_" + self.custom_Speed_Naming + "_" + \
                    str(self.timespan_url).replace("-", "_to_") + curDate
            ...
            # If the speed combobox does not display "Custom..."...
                ...
                # This will be the naming convention of the output feature class
                # once converted from CSV.
                self.nameFeatureClass_FromCSV = noaa + "_" + wind + "_" + \
                                self.mag_naming + "_" + \
                                str(self.timespan_url).replace("-", "_to_") + \
                                curDate
            ...
            self.func_Scroll_setOutputText("Checking CSV file for missing "
                                    ...
                                    "removing invalid wind speeds.", None)

            # Remove single quotes and spaces from the CSV's first row.
            firstLine = \
                str(next(self.csv_Reader)).replace("'", "").replace(" ", "")

            # If the wind headers exist in that CSV's first row...
            if wind_headers in firstLine:

                # Write the wind headers to the output CSV file, splitting
                # at the comma. The firstLine row will be skipped for the next
                # step.
                self.csv_Writer.writerow(wind_headers.split(","))

            else:

                # Else, if the wind headers are not present in the CSV's
                # first row, add the wind headers.
                self.csv_Writer.writerow(wind_headers.split(","))
            ...
            for row in self.csv_Reader:
                ...
                # If entire row is not empty and speed column and lat/long
                ...
                if row and row[10] != "" and row[15] != "" and row[16] != "":
                    ...
                    if (row[15] != "0" or row[15] != "0.0") and \
                        ...
                        if float(row[15]) >= -90.0 and float(row[15]) <= 90.0 \
                            ...
                            # If the speed is at least 30 and does not
                            # exceed 200 (any values below 30 knots are
                            # ignored).
                            # Note: Because zeroes exist within the CSV
                            # files, those values must first be converted to
                            # float and then to integer as they are being
                            # read. Otherwise, the process will fail due to
                            # Python not being able to convert 0.0 directly
                            # to integer).
                            if int(float(row[10])) >= 30 and \
                                    int(float(row[10])) <= 200:

                                # If the wind speed is set to All...
                                if self.stringWindSpeed.get() == "All":

                                    # Write the row to the output CSV.
                                    self.csv_Writer.writerow(row)

                                # If the wind speed is set to 30-60...
                                elif self.stringWindSpeed.get() == "30 - 60":

                                    # If the CSV row speed is at least 30 and
                                    # less than 60...
                                    if int(float(row[10])) >= 30 and \
                                            int(float(row[10])) < 60:
                                ...
                                # If the wind speed is set to 60-90...
                                elif self.stringWindSpeed.get() == "60 - 90":

                                    # If the CSV row speed is at least 60 and
                                    # less than 90...
                                    if int(float(row[10])) >= 60 and \
                                            int(float(row[10])) < 90:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the wind speed is set to 90-120...
                                elif self.stringWindSpeed.get() == "90 - 120":

                                    # If the CSV row speed is at least 90 and
                                    # less than 120...
                                    if int(float(row[10])) >= 90 and \
                                            int(float(row[10])) < 120:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the wind speed is set to 120+...
                                elif self.stringWindSpeed.get() == "120+":

                                    # If the CSV row speed is at least 120...
                                    if int(float(row[10])) >= 120:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the wind speed is set to Custom...
                                elif self.stringWindSpeed.get() == "Custom...":

                                    # If the CSV row speed is between the
                                    # Speed From/To selections...
                                    if int(float(row[10])) >= \
                                            int(self.textCustomSpeedFrom) and \
                                            int(float(row[10])) <= \
                                            int(self.textCustomSpeedTo):
            ...
            # Run function to check if output CSV file contains 0 wind.
    ...
    def func_CustomTimespan_CSV_HeaderCheck_ValueCheck(self):
        ...
        try:
            ...
            if self.stringWindSpeed.get() == "Custom...":
                ...
                # once converted from CSV.
                self.nameFeatureClass_FromCSV = \
                    noaa + "_" + wind + "_" + self.custom_Speed_Naming + \
                    self.timespan_file_folder_naming + curDate
            ...
            # If the speed combobox does not display "Custom..."...
                ...
                self.csv_OutputFile = open(self.csvDirectory + "/" + noaa +
                            "_" + wind + "_" +
                            self.mag_naming + self.timespan_file_folder_naming +
                            curDate + "_checked" + fileExtCSV, "w")
            ...
            for row in self.csv_Reader:
                ...
                if row and fromDate <= date_in_row <= toDate:
                    ...
                    # If speed column and lat/long columns are not blank...
                        ...
                        if (row[15] != "0" or row[15] != "0.0") and \
                            ...
                            if float(row[15]) >= -90.0 and \
                                ...
                                if int(float(row[10])) >= 30 and \
                                    ...
                                    # If wind speed selection is All...
                                    if self.stringWindSpeed.get() == "All":

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                    # If wind speed selection is 30-60...
                                    elif self.stringWindSpeed.get() == \
                                            "30 - 60":

                                        # If wind speed in CSV is within this
                                        # range...
                                        if int(float(row[10])) >= 30 and \
                                                int(float(row[10])) < 60:
                                    ...
                                    # If wind speed selection is 60-90...
                                    elif self.stringWindSpeed.get() == \
                                            "60 - 90":
                                    ...
                                    # If wind speed selection is 90-120
                                    elif self.stringWindSpeed.get() == \
                                            "90 - 120":
                                    ...
                                    # If wind speed selection is 120+...
                                    elif self.stringWindSpeed.get() == "120+":
                                    ...
                                    # If wind speed selection is Custom...
                                    elif self.stringWindSpeed.get() == \
                                            "Custom...":

                                        # If CSV row's wind speed is between
    ...
    def func_Check_If_Empty_CSV(self, csvName):
        ...
        try:
            ...
            with open(csvName) as csvFile:
                ...
                # to a raw wind count variable.
                self.raw_wind_count = sum(1 for row in csvFile)

                # If the wind count is less than 1...
                if self.raw_wind_count <= 1:

                    # Display error message explaining that no wind
                    ...
                    # Else if wind records exist, but are in excess of
                    # 500,000 records...
                    if self.raw_wind_count >= 500000:
    ...
    def func_CreateFeatureClass_from_CSV(
        ...
        try:
            ...
            # class.
            self.featureClass_Wind_Unclipped_Unprojected = \
            ...
            self.featureClass_Wind_Unclipped_Projected = \
                self.featureClass_Wind_Unclipped_Unprojected + \
                "_Projected"
            ...
            arcpy.Project_management(
                self.featureClass_Wind_Unclipped_Unprojected,
                self.featureClass_Wind_Unclipped_Projected, pcsReference)
            ...
            self.func_Scroll_setOutputText("Renaming feature class " +
                        self.featureClass_Wind_Unclipped_Projected + " to " +
                        self.featureClass_Wind_Unclipped_Unprojected + "...",
                        None)
    ...
    def func_RadioButton_Clipping_Command(self):
        ...
        try:
            ...
            if self.radioButton_Selection == 3 and \
                ...
                # and clear county variables to prevent duplicate drop-down lists
    ...
    def func_RadioButton_Clipping_Rules(self):
        ...
        try:
            ...
            if self.radioButton_Selection == 1:
                ...
                # Then execute function to clip wind features to the
                ...
                # Execute function to verify if clipped wind feature class
    ...
    def func_FeatureClass_Clip(self, inputFC, clipFC, outputFC_Name):
        ...
        # wind point feature class to the user-selected Census polygon
        ...
        try:
            ...
            if self.radioButton_Selection == 1:
                ...
                self.func_Scroll_setOutputText(
                    "Clipping wind features to 50 states and DC...", None)
            ...
            # If the user selects State...
                ...
                # Display this particular message before executing the clip.
                self.func_Scroll_setOutputText(
                    "Clipping wind features to state selection(" +
            ...
            # If the user selects County...
                ...
                # Display this particular message before executing the clip.
                self.func_Scroll_setOutputText(
                    "Clipping wind features to " +
                    self.stringCounty_Name.get() + ", " +
                    self.stringState_Name.get() + "...", None)

            # Perform a clip of the wind point features to the specified
            # polygon boundary.
            ...
            if self.radioButton_Selection == 1:
                ...
                self.func_Scroll_setOutputText("Wind features have been "
                                               "clipped to 50 states and DC.",
                                               None)
    ...
    def func_Check_If_Empty_Output_FeatureClass(self,
        ...
        # wind point feature class have features once clipped.
        ...
        # variable.
        self.windCountResult = \
            arcpy.GetCount_management(clipped_FeatureClass_Points)

        # Store the integer version of the wind count to variable.
        self.clipped_windCount = \
            int(self.windCountResult.getOutput(0))

        # If user selects USA...
        if self.radioButton_Selection == 1:
            # Display this particular message with wind count value.
            self.func_Scroll_setOutputText(str(self.clipped_windCount) +
                        " wind features observed within the 50 states and DC.",
                        None)
        ...
        # If zero wind features present within the feature class...
        if self.clipped_windCount < 1:
            # Display this message and error message to the user, since the
            # script can't proceed any farther.
            self.func_Scroll_setOutputText("1 or fewer wind features "
                            "present within the timespan/speed/location.\n" +
                            "Unable to continue with analysis.",
                            color_Red)

            messagebox.showerror(errorMessage_Header,
                                 "No wind features present within the "
                                 "timespan/speed/location.\n" +
                                 "Unable to continue with analysis.")
    ...
    def func_Controls_For_Analysis_Options(self, featureClass_Mask):
        ...
        try:
            ...
            if self.statusAnalysis_OutputToCSVFile.get() == 1:
                ...
                if self.radioButton_Selection == 1:
                    ...
                    # Perform spatial join of clipped wind feature class
                    ...
                    # If timespan and speed drop-downs don't show Custom...
                    if self.stringWindTimespan.get() != "Custom..." and \
                            self.stringWindSpeed.get() != "Custom...":
                        # Write the following lines into the data counts CSV.
                        csvFile.write("Timespan:," +
                                      self.stringWindTimespan.get() + ",\n")
                        csvFile.write("Speed Range (Knots):," +
                                      self.stringWindSpeed.get() + ",\n")

                    # If timespan drop-down shows Custom and speed doesn't..
                    if self.stringWindTimespan.get() == "Custom..." and \
                            self.stringWindSpeed.get() != "Custom...":
                        # WRite the following lines into the data counts CSV.
                    ...
                    # If timespan and speed drop-downs both show Custom...
                    if self.stringWindTimespan.get() == "Custom..." and \
                            self.stringWindSpeed.get() == "Custom...":
                        # Write the following lines into the data counts CSV.
                        csvFile.write("Timespan:," + self.textCustomYearFrom +
                                      ...
                                      self.textCustomMonthTo.zfill(2) + ",\n")
                        csvFile.write("Speed Range (Knots):," +
                                      self.textCustomSpeedFrom + "-" +
                                      self.textCustomSpeedTo + ",\n")

                    # If timespan drop-down doesn't show Custom and Speed
                    # does...
                    if self.stringWindTimespan.get() != "Custom..." and \
                            self.stringWindSpeed.get() == "Custom...":
                        # Write the following lines into the data counts CSV.
                    ...
                    # timespan/speed selection.
                    csvFile.write("\n")
                    csvFile.write("USA (and DC) Wind:," +
                                  str(self.clipped_windCount) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("Nationwide Wind Data,\n")
                    csvFile.write("V,V,V,V,V,\n")
                    csvFile.write("Minimum Speed,Maximum Speed,"
                                  "Mean Speed,Median Speed,"
                                  "Mode Speed,\n")

                    # Create empty list to store all US speed values.
                    magList_US = []

                    # Open the in_memory spatial join as a Search Cursor,
                    # with analysis fields set to FIPS code and speed.
                    with arcpy.da.SearchCursor("in_memory/SpatialJoin",
                        ...
                        for row in cursor:
                            ...
                            # Add the speed to the US list.
                            magList_US.append(row[1])

                    try:

                        # Calculate mode of all speed within US list.
                    ...
                    # Min speed, max speed, mean speed, median speed, and mode
                    # speed.
                    ...
                    csvFile.write("\n")
                    csvFile.write("Wind Data per State,\n")
                    csvFile.write("V,V,V,V,V,V,V,V,\n")
                    csvFile.write("States with Wind,Wind Count,"
                                  "Minimum Speed,Maximum Speed,"
                                  "Mean Speed,Median Speed,"
                                  "Mode Speed,State FIPS Code,\n")

                    # Create empty list for storing wind/speed values.
                    magList = []

                    # For each state value in the ordered dictionary...
                    for key in dictKeys_OrderedDict_StateNames:

                        # Set counter to zero, for counting wind within
                        ...
                        if counter > 0:
                            ...
                            try:
                                ...
                                # Calculate mode of all speeds within list.
                            ...
                            # Write the following data to the data counts CSV.
                            # State name, how many wind, min mag, max
                ...
                if self.radioButton_Selection == 2:
                    ...
                    # Execute function to obtain total wind count within
                    # the US.
                    ...
                    # If timespan drop-down shows Custom and speed drop-down
                    # doesn't show Custom...
                    if self.stringWindTimespan.get() == "Custom..." and \
                            self.stringWindSpeed.get() != "Custom...":
                    ...
                    csvFile.write("\n")
                    csvFile.write("USA (and DC) Wind:," +
                                  str(self.usa_wind_count) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("State Wind - " +
                                  ...
                                  ",\n")
                    csvFile.write("\n")
                    csvFile.write("Statewide Wind Data,\n")
                    csvFile.write("V,V,V,V,V,\n")
                    csvFile.write("Minimum Speed,Maximum Speed,Mean Speed,"
                                  "Median Speed,Mode Speed,\n")

                    # Create empty list to store all state speed values.
                    magList_State = []
                    ...
                    with arcpy.da.SearchCursor("in_memory/SpatialJoin",
                        ...
                        for row in cursor:
                            ...
                            # Add the speed to the state list.
                            magList_State.append(row[1])

                    try:

                        # Calculate mode of all speeds within state list.
                    ...
                    csvFile.write("\n")
                    csvFile.write("Wind Data per County\n")
                    csvFile.write("V,V,V,V,V,V,V,\n")
                    csvFile.write("Counties with Wind,Wind Count,"
                                  "Minimum Speed,Maximum Speed,"
                                  "Mean Speed,Median Speed,"
                                  "Mode Speed,\n")
                    ...
                    for key in GUI_CountiesPerState.dict_state_counties[
                        ...
                        # Open the in_memory spatial join as a Search Cursor,
                        # with analysis fields set to county name and speed.
                        with arcpy.da.SearchCursor("in_memory/SpatialJoin",
                                            [census_Shapefile_Field_CountyName,
                                            analysis_Mag_Field]) as cursor:
                        ...
                        if counter > 0:
                            ...
                            # Write the following data to the data counts CSV.
                            # County name, how many wind, min mag, max
                            # mag, mean mag, median mag, and mode mag.
                            csvFile.write(str(key) + "," + str(counter) +
                                "," + str("{0:.2f}".format(min(magList))) +
                ...
                if self.radioButton_Selection == 3:
                    ...
                    # timespan drop-down shows Custom and speed drop-down
                    # shows Custom...
                    if self.stringWindTimespan.get() == "Custom..." and \
                            self.stringWindSpeed.get() == "Custom...":
                    ...
                    csvFile.write("State Wind - " +
                                ...
                                str(dictKeys_OrderedDict_StateNames[
                                self.stringState_Name.get()]) + "):," +
                                str(self.state_wind_count) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("County Wind - " +
                                  self.stringCounty_Name.get() + ":," +
                                  str(self.clipped_windCount) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("County Wind Data\n")
                    csvFile.write("V,V,V,V,V,\n")
                    csvFile.write("Minimum Speed,Maximum Speed,"
                                  "Mean Speed,Median Speed,"
                                  "Mode Speed,\n")
        ...
        try:
            ...
            if self.statusAnalysis_KernelDensity.get() == 1:
                ...
                self.func_Scroll_setOutputText("Performing Kernel Density "
                                            "Analysis on all points, "
                                            "then clipping/masking output to " +
                                            featureClass_Mask + " extent...",
                                            None)
        ...
        try:
            ...
            if self.statusAnalysis_NaturalNeighbor.get() == 1:
                ...
                # If the total number of wind features reach 14,750,000...
                if self.raw_wind_count >= 14750000:
                    ...
                    self.func_Scroll_setOutputText(
                        ...
                        "input wind count nears 15,000,000.\n" +
        ...
        try:
            ...
            if self.statusAnalysis_OptHotSpot.get() == 1:
                ...
                # If the clipped wind count is less than 30 features...
                if self.clipped_windCount < 30:
                    ...
                    self.func_Scroll_setOutputText(
                        ...
                        " 30 wind within the sampling area.\n" +
                        "Skipping this analysis...", color_Red)
                ...
                # If the clipped wind count is less than 60 features...
                if self.clipped_windCount < 60:
                    ...
                    self.func_Scroll_setOutputText(
                        ...
                        " ANALYSIS FIELD OPTION):\n" +
                        "According to Esri, this analysis requires a minimum of"
                        " 60 wind within the sampling area.\n" +
                        "Skipping this analysis...", color_Red)
                    ...
                    self.func_Scroll_setOutputText("Optimized Hot Spot Analysis"
                                " on all points (no analysis field) completed.",
                                None)

                    self.func_Scroll_setOutputText("Clipping Optimized Hot Spot"
                                    " Analysis (no analysis field) results...",
                                    None)
                    ...
                    self.func_Scroll_setOutputText("Clipping for Optimized Hot "
                                "Spot Analysis (no analysis field) completed.",
                                None)
        ...
        try:
            ...
            if self.statusAnalysis_Trend.get() == 1:
                ...
                output_Trend = arcpy.sa.Trend(required_InputFile,
                                              required_ZField,
                                              optional_CellSize, optional_Order,
                                              optional_RegressionType,
                                              optional_OutputRMSFile)
    ...
    def func_CSV_DataCount_Nationwide_Count(self):
        ...
        # This function controls the counting of all wind within the US.
        ...
        try:
            ...
            # Create new feature class variable in_memory for clipped wind
            # points.
            clipped_Output_FeatureClass = "in_memory/ClippedOutputFC_US"

            # Perform a clip of raw wind points to the 50 US states and DC
            ...
            # Perform a count on the clipped wind features found within
            # the in_memory clipped feature class.
            windCountResult_US = arcpy.GetCount_management(
                clipped_Output_FeatureClass)

            # Store the integer wind count as a variable for use within
            # other functions.
            self.usa_wind_count = \
                int(windCountResult_US.getOutput(0))
    ...
    def func_CSV_DataCount_Statewide_Count(self):
        ...
        # This function controls the counting of all wind within the
        # user-selected state.
        ...
        try:
            ...
            # Perform a clip of raw wind points to the user-selected state
            ...
            # Perform a count on the clipped wind features found within
            # the in_memory clipped feature class.
            windCountResult_State = arcpy.GetCount_management(
                clipped_Output_FeatureClass)

            # Store the integer wind count as a variable for use within
            # the in_memory clipped feature class.
            self.state_wind_count = \
                int(windCountResult_State.getOutput(0))
    ...
    def func_windowResize(self):
        ...
        self.checkboxExpansionWidth = guiWindow_WindOptions_Width + 300

        # Set the GUI expansion height if timespan/speed drop-downs show
        # "Custom...".
        self.customSelection_Height = guiWindow_WindOptions_Height + 75

        # If the More Options checkbox is not selected and "Custom..." is not
        # selected for both timespan and speed...
        if self.statusVar_Checkbutton_Options.get() == 0 and \
                self.stringWindTimespan.get() != "Custom..." and \
                self.stringWindSpeed.get() != "Custom...":
            ...
            # If the Custom Speed Min/Max drop-down lists are visible...
            if self.comboFrame_Custom_Speed is not None:

                # Remove the Custom Speed Min/Max drop-down lists.
                self.comboFrame_Custom_Speed.grid_remove()
        ...
        # or custom speed are selected...
        elif self.statusVar_Checkbutton_Options.get() == 0 and \
                (self.stringWindTimespan.get() == "Custom..." or
                 self.stringWindSpeed.get() == "Custom..."):
        ...
        # timespan and custom speed are not selected...
    ...
    def func_Enable_Buttons(self):
        ...
        # Enable Speed drop-down list.
        self.combo_WindSpeed.config(state="readonly")
        self.combo_WindSpeed.update()

        # Enable Timespan drop-down list.
        self.combo_WindTimespan.config(state="readonly")
        self.combo_WindTimespan.update()
        ...
        if self.comboFrame_Custom_Speed is not None:
            ...
            # Enable the Custom Speed Min drop-down list.
            self.combo_Custom_Speed_Min.config(state="readonly")
            self.combo_Custom_Speed_Min.update()

            # Enable the Custom Speed Max drop-down list.
            self.combo_Custom_Speed_Max.config(state="readonly")
            self.combo_Custom_Speed_Max.update()
    ...
    def func_Disable_Buttons(self):
        ...
        # Disable the Speed drop-down list.
        self.combo_WindSpeed.config(state=tkinter.DISABLED)
        self.combo_WindSpeed.update()

        # Disable the Timespan drop-down list.
        self.combo_WindTimespan.config(state=tkinter.DISABLED)
        self.combo_WindTimespan.update()
        ...
        if self.comboFrame_Custom_Speed is not None:
            ...
            # Disable the Min Speed drop-down list.
            self.combo_Custom_Speed_Min.config(state=tkinter.DISABLED)
            self.combo_Custom_Speed_Min.update()

            # Disable the Max Speed drop-down list.
            self.combo_Custom_Speed_Max.config(state=tkinter.DISABLED)
            self.combo_Custom_Speed_Max.update()
