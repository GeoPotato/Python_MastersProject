...
# The following code represents the functionality for Tornado Options.
...
guiWindow_TornadoOptions_Width = 470
guiWindow_TornadoOptions_Height = 225

# Formula to acquire date from one year ago in YYYY format. NOAA's portal for
# downloading tornado data may take over a year to have latest data available.
...
# Tuple for populating tornado timespan text values within the combo box.
textCombo_TornadoTimespan = ()
textCombo_TornadoTimespan = textCombo_TornadoTimespan + \
                         ("Select...", "All | 1950-" + str(int_OneYearAgo))
...
while int_YearIterator >= 2008:
    textCombo_TornadoTimespan = \
        textCombo_TornadoTimespan + (str(int_YearIterator),)
...
textCombo_TornadoTimespan = textCombo_TornadoTimespan + ("2005-2007",
                                                         "2000-2004", "90-99",
                                                         "80-89", "70-79",
                                                         "60-69", "50-59",
                                                         "Custom...")
...
# The following code populates a list of years beginning with 1950 and ending
# with one year ago. This list will be used to populate the custom timespan
# combo box.
intList_Years = []
while int_YearIterator >= 1950:
    intList_Years.append(str(int_YearIterator))
    int_YearIterator = int_YearIterator - 1

# Tuple for populating tornado intensity text values within the combo box.
textCombo_TornadoIntensity = ("Select...", "All", "EF-0", "EF-1", "EF-2",
                              "EF-3", "EF-4", "EF-5", "Custom...")
...
int_Combo_Intensities = (0, 1, 2, 3, 4, 5)
...
# tornado CSVs from NOAA website (used for non-custom parameter inputs only).
tornado_siteURL = "http://www.spc.noaa.gov/wcm/data/"

# Comma-delimited header names for the NOAA tornado CSVs.
tornado_headers = "om" + "," + "yr" + "," + "mo" + "," + "dy" + "," + "date" + \
               "," + "time" + "," + "tz" + "," + "st" + "," + "stf" + "," + \
               "stn" + "," + "mag" + "," + "inj" + "," + "fat" + "," + \
...
# Column header from CSV file for intensity, used as an input parameter for the
...
try:
    ...
    # tornado and polygon feature classes.
...
# Class for the Tornado Options GUI functionality.
class TornadoOptions(FrameLifts):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Settings and configuration for the Tornado Options GUI window.
        self.winfo_toplevel().title("Tornado Options")
        self.winfo_toplevel().geometry("%dx%d" %
                                       (guiWindow_TornadoOptions_Width,
                                        guiWindow_TornadoOptions_Height))
        ...
        # Frame that displays the initial Tornado dialog layout.
    ...
    def func_ComboFrame(self):
        ...
        self.stringTornadoTimespan = tkinter.StringVar()
        self.combo_TornadoTimespan = ttk.Combobox(self.comboFrame, width=13,
                                        textvariable=self.stringTornadoTimespan,
                                        state="readonly")
        self.combo_TornadoTimespan["values"] = (textCombo_TornadoTimespan)
        self.combo_TornadoTimespan.grid(column=0, row=1, sticky=tkinter.W)

        # GIF icon to symbolize Tornado within the GUI.
        dirFolder = os.path.dirname(__file__)
        gifPath = os.path.join(dirFolder, "Icon_Tornado.gif")
        self.photo_Tornado_Icon = tkinter.PhotoImage(file=gifPath)
        colorCode_Black = "black"
        self.subSampleImage = self.photo_Tornado_Icon.subsample(4, 4)
        self.label_for_icon = ttk.Label(self.comboFrame, borderwidth=2,
                                        relief="solid",
                                        background=colorCode_Black,
                                        image=self.subSampleImage)
        self.label_for_icon.photo = self.photo_Tornado_Icon
        self.label_for_icon.grid(column=1, row=0, padx=0,
                                 pady=5, sticky=tkinter.EW, rowspan=2)

        # Combobox requirements for Intensity.
        self.stringTornadoIntensity = tkinter.StringVar()
        self.combo_TornadoIntensity = ttk.Combobox(self.comboFrame, width=12,
                                    textvariable=self.stringTornadoIntensity,
                                    state="readonly")
        self.combo_TornadoIntensity["values"] = \
            (textCombo_TornadoIntensity)
        self.combo_TornadoIntensity.grid(column=2, row=1, sticky=tkinter.W)

        # Combobox selection binding that will be used to populate URL Field.
        self.combo_TornadoTimespan.bind("<<ComboboxSelected>>",
                                     self.func_PopulateURLField_Timespan)

        # Combobox selection binding that will be used to populate URL Field.
        self.combo_TornadoIntensity.bind("<<ComboboxSelected>>",
        ...
        self.combo_TornadoTimespan.current(0)

        # Display first index for Intensity (Select...).
        self.combo_TornadoIntensity.current(0)
    ...
    def func_ComboCustomIntensity(self):
        ...
        self.comboLabelFrame_Custom_Intensity = ttk.LabelFrame(
            self.comboFrame_Custom_Intensity, text="Range:",
            labelanchor=tkinter.N)
        ...
        self.combo_Custom_Intensity_Min["values"] = (int_Combo_Intensities)
        ...
        self.combo_Custom_Intensity_Max["values"] = (int_Combo_Intensities)
        ...
        # Display first index for MINIMUM EF intensity (0).
        self.combo_Custom_Intensity_Min.current(0)

        # Display last index for MAXIMUM EF intensity (5).
        self.combo_Custom_Intensity_Max.current(5)
    ...
    def func_URLFrame(self):
        ...
        urlLabel_TornadoURL = ttk.Label(self.urlFrame,
                                     text="URL Download File Link:")
        urlLabel_TornadoURL.grid(column=0, row=0, padx=20, pady=1,
                              sticky=tkinter.W)

        # Parameters for the URL textbox.
        self.textURLTextbox = tkinter.StringVar()
        self.textURLTextbox.set(urlDialog_Message)
        urlTextbox_TornadoURL = ttk.Entry(self.urlFrame,
                                       textvariable=self.textURLTextbox,
                                       width=70, state="readonly")
        urlTextbox_TornadoURL.grid(column=0, row=1, padx=20, pady=0)
    ...
    def func_PopulateURLField_Timespan(self, event):
        ...
        try:
            ...
            if self.stringTornadoTimespan.get() == "Select...":
            ...
            # If the user selects all tornado data from combobox...
            elif self.stringTornadoTimespan.get() == \
                    "All | 1950-" + str(int_OneYearAgo):

                # Assign/clear variables as needed.
                self.timespan_url = "1950-" + str(int_OneYearAgo) #+ "_torn"

                # Set the URL text of non custom timespan/intensity values.
                self.text_Tornado_URL = tornado_siteURL + self.timespan_url + \
                                        "_" + torn + fileExtCSV + fileExtZip

                if self.stringTornadoIntensity.get() != "Select...":
                    self.textURLTextbox.set(self.text_Tornado_URL)
            ...
            elif self.stringTornadoTimespan.get() == "Custom...":
            ...
            # If the user selects any option aside from Select, All, or Custom..
                ...
                # Assign/clear variables as needed.
                self.timespan_url = \
                    self.stringTornadoTimespan.get() #+ "_torn"
    ...
    def func_PopulateURLField_Intensity(self, event):
        ...
        elif self.stringTornadoIntensity.get() == "All":
        ...
        # If the user selects EF-0 from the combobox...
        elif self.stringTornadoIntensity.get() == "EF-0":
        ...
        # If the user selects EF-1 from the combobox...
        elif self.stringTornadoIntensity.get() == "EF-1":
        ...
        # If the user selects EF-2 from the combobox...
        elif self.stringTornadoIntensity.get() == "EF-2":
        ...
        # If the user selects EF-3 from the combobox...
        elif self.stringTornadoIntensity.get() == "EF-3":
        ...
        # If the user selects EF-4 from the combobox...
        elif self.stringTornadoIntensity.get() == "EF-4":
        ...
        # If the user selects EF-5 from the combobox...
        elif self.stringTornadoIntensity.get() == "EF-5":
        ...
        # If the user selects "Custom..." from the combobox...
        elif self.stringTornadoIntensity.get() == "Custom...":

            # Assign/clear variables as needed.
            # Custom tornado intensity will default to All, as all data is
    ...
    def func_WorkspaceFolderFrame(self):
        ...
        self.workspaceFolderTextbox_TornadoFolderDialog = \
            ttk.Entry(self.workspaceFolderFrame,
                      textvariable=self.textWorkspaceDialogPath, width=52,
                      state="readonly")
        self.workspaceFolderTextbox_TornadoFolderDialog.grid(column=1,
                                                          row=1, padx=0, pady=0,
                                                          sticky=tkinter.W)
    ...
    def func_OKClick(self):
        ...
        try:
            ...
            self.okClick_NonCustom_ExpansionHeight = \
                guiWindow_TornadoOptions_Height + 200
            ...
            if self.stringTornadoTimespan.get() != "Select..." and \
                    self.stringTornadoIntensity.get() != "Select..." and \
                    self.workspaceFolderTextbox_TornadoFolderDialog.get() != \
                ...
                if self.statusVar_Checkbutton_Options.get() == 1:
                    ...
                    if self.stringTornadoTimespan.get() == "Custom..." or \
                            self.stringTornadoIntensity.get() == "Custom...":
                ...
                elif self.stringTornadoIntensity.get() == "Select...":
                ...
                elif self.workspaceFolderTextbox_TornadoFolderDialog.get() == \
                                folderDialog_Message:
    ...
    def func_CreateFolder(self):
        ...
        try:
            ...
            if self.statusVar_Checkbutton_Options.get() == 1:
                ...
                if self.stringTornadoTimespan.get() != "Custom..." and \
                        self.stringTornadoIntensity.get() != "Custom...":
    ...
    def func_DownloadCSV(self):
        ...
        try:
            ...
            if self.stringTornadoTimespan.get() != "Custom...":

                # If timespan equals All...
                if self.stringTornadoTimespan.get() == \
                        "All | 1950-" + str(int_OneYearAgo):
                    ...
                    try:
                        ...
                        try:
                            ...
                            # 20180424 - PLEASE NOTE:
                            # NOAA added 2017 data to the website on 20180410,
                            # but added "_prelim" to the naming convention for
                            # 2017 data. They have stated that this will change
                            # soon, so for now, a modified URL string will
                            # retrieve this preliminary data if the regular URL
                            # does not work. This is a temporary fix and will
                            # likely need to be deleted once the web URL has
                            # been updated to its regular naming convention.
                            temp_PreliminaryURL = \
                                self.textURLTextbox.get()[:47] + "_prelim" + \
                                self.textURLTextbox.get()[47:]

                            request.urlretrieve(temp_PreliminaryURL,
                                self.csvDirectory + "/" + noaa + "_" + torn +
                                "_" +
                                str(self.timespan_url).replace("-", "_to_") +
                                curDate + fileExtCSV + fileExtZip)
                    ...
                    try:
                        ...
                        os.rename(self.csvDirectory + self.timespan_url + "_" +
                              torn + fileExtCSV, self.csvDirectory + noaa +
                              "_" + torn + "_" +
                              str(self.timespan_url).replace("-", "_to_") +
                              curDate + fileExtCSV)
                        ...
                        # but added "_prelim" to the naming convention for
                        # 2017 data. They have stated that this will change
                        # soon, so for now, a modified string will
                        # rename this preliminary CSV file if the regular
                        # CSV file name can't be located. This is a
                        # temporary fix and will likely need to be deleted
                        # once the web URL has been updated to its regular
                        # naming convention.
                        os.rename(self.csvDirectory + self.timespan_url + "_" +
                                torn + "_prelim" + fileExtCSV,
                                self.csvDirectory + noaa +
                                "_" + torn + "_" +
                                str(self.timespan_url).replace("-", "_to_") +
                                curDate + fileExtCSV)
                    ...
                    try:
                        ...
                        try:
                            ...
                            # likely need to be deleted once the web URL has
                            # been updated to its regular naming convention.
                            #http://www.spc.noaa.gov/wcm/data/2017_torn.csv
                            temp_PreliminaryURL = \
                                self.textURLTextbox.get()[:42] + "_prelim" + \
                                self.textURLTextbox.get()[42:]
            ...
            if self.stringTornadoTimespan.get() == "Custom...":
                ...
                # Unzip the CSV file within the same folder location.
                unZipThisFile = zipfile.ZipFile(self.csvDirectory + "/" + noaa +
                                                "_" + torn + "_" +
                                str(self.timespan_url).replace("-", "_to_") +
                                curDate + fileExtCSV + fileExtZip, 'r')
    ...
    def func_NonCustomTimespan_CSV_HeaderCheck_ValueCheck(self):
        ...
        try:
            ...
            # If the intensity combobox displays "Custom..."...
            if self.stringTornadoIntensity.get() == "Custom...":
                ...
                self.nameFeatureClass_FromCSV = noaa + "_" + torn + "_" + \
                        self.custom_Intensity_Naming + "_" + \
                        str(self.timespan_url).replace("-", "_to_") + curDate

            # If the intensity combobox does not display "Custom..."...
            elif self.stringTornadoIntensity.get() != "Custom...":
            ...
            self.func_Scroll_setOutputText("Checking CSV file for missing "
                                           "headers, erroneous lat/long values,"
                                           " and removing invalid tornado "
                                           "sizes.", None)
            ...
            # If the tornado headers exist in that CSV's first row...
            if tornado_headers in firstLine:

                # Write the tornado headers to the output CSV file, splitting
                # at the comma. The firstLine row will be skipped for the next
                # step.
                self.csv_Writer.writerow(tornado_headers.split(","))

            else:

                # Else, if the tornado headers are not present in the CSV's
                # first row, add the tornado headers.
                self.csv_Writer.writerow(tornado_headers.split(","))
            ...
            for row in self.csv_Reader:
                ...
                # If entire row is not empty and intensity column and lat/long
                ...
                if row and row[10] != "" and row[15] != "" and row[16] != "":
                    ...
                    if (row[15] != "0" or row[15] != "0.0") and \
                        ...
                        if float(row[15]) >= -90.0 and float(row[15]) <= 90.0 \
                            ...
                            # If the intensity is at least zero and less
                            # than 6 (negative values and above five indicate
                            # errors)...
                            if int(row[10]) >= 0 and int(row[10]) < 6:

                                # If the tornado intensity is set to All...
                                if self.stringTornadoIntensity.get() == "All":

                                    # Write the row to the output CSV.
                                    self.csv_Writer.writerow(row)

                                # If the tornado intensity is set to EF-0...
                                elif self.stringTornadoIntensity.get() == \
                                        "EF-0":

                                    # If the CSV row intensity is 0...
                                    if int(row[10]) == 0:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the tornado intensity is set to EF-1...
                                elif self.stringTornadoIntensity.get() == \
                                        "EF-1":

                                    # If the CSV row intensity is 1...
                                    if int(row[10]) == 1:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the tornado intensity is set to EF-2...
                                elif self.stringTornadoIntensity.get() == \
                                        "EF-2":

                                    # If the CSV row intensity is 2...
                                    if int(row[10]) == 2:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the tornado intensity is set to EF-3...
                                elif self.stringTornadoIntensity.get() == \
                                        "EF-3":

                                    # If the CSV row intensity is 3...
                                    if int(row[10]) == 3:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the tornado intensity is set to EF-4...
                                elif self.stringTornadoIntensity.get() == \
                                             "EF-4":

                                    # If the CSV row intensity is 4...
                                    if int(row[10]) == 4:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the tornado intensity is set to EF-5...
                                elif self.stringTornadoIntensity.get() == \
                                             "EF-5":

                                    # If the CSV row intensity is 5...
                                    if int(row[10]) == 5:

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                # If the tornado intensity is set to Custom...
                                elif self.stringTornadoIntensity.get() == \
                                        "Custom...":

                                    # If the CSV row intensity is between the
                                    # Intensity From/To selections...
                                    if int(row[10]) >= \
                                        int(self.textCustomIntensityFrom) and \
                                        int(row[10]) <= \
            ...
            # Run function to check if output CSV file contains 0 tornado.
        ...
        try:
            ...
            if self.stringTornadoIntensity.get() == "Custom...":
                ...
                self.nameFeatureClass_FromCSV = \
                    noaa + "_" + torn + "_" + self.custom_Intensity_Naming + \
                    self.timespan_file_folder_naming + curDate
            ...
            for row in self.csv_Reader:
                ...
                if row and fromDate <= date_in_row <= toDate:
                    ...
                    # If intensity column and lat/long columns are not blank...
                    if row[10] != "" and row[15] != "" and row[16] != "":
                        ...
                        if (row[15] != "0" or row[15] != "0.0") and \
                            ...
                            if float(row[15]) >= -90.0 and \
                                ...
                                # If the intensity is at least zero and
                                # less than 6 (negative values and values above
                                # five indicate errors).
                                ...
                                # to integer).
                                if int(float(row[10])) >= 0 and \
                                        int(float(row[10])) < 6:

                                    # If tornado intensity selection is All...
                                    if self.stringTornadoIntensity.get() == \
                                            "All":

                                        # Write the row to the output CSV.
                                        self.csv_Writer.writerow(row)

                                    # If tornado intensity selection is EF-0...
                                    elif self.stringTornadoIntensity.get() == \
                                            "EF-0":

                                        # If tornado intensity in CSV is zero...
                                        if int(float(row[10])) == 0:

                                            # Write the row to the output CSV.
                                            self.csv_Writer.writerow(row)

                                    # If tornado intensity selection is EF-1...
                                    elif self.stringTornadoIntensity.get() == \
                                            "EF-1":

                                        # If tornado intensity in CSV is 1...
                                        if int(float(row[10])) == 1:

                                            # Write the row to the output CSV.
                                            self.csv_Writer.writerow(row)

                                    # If tornado intensity selection is EF-2...
                                    elif self.stringTornadoIntensity.get() == \
                                            "EF-2":

                                        # If tornado intensity in CSV is 2...
                                        if int(float(row[10])) == 2:

                                            # Write the row to the output CSV.
                                            self.csv_Writer.writerow(row)

                                    # If tornado intensity selection is EF-3...
                                    elif self.stringTornadoIntensity.get() == \
                                            "EF-3":

                                        # If tornado intensity in CSV is 3...
                                        if int(float(row[10])) == 3:

                                            # Write the row to the output CSV.
                                            self.csv_Writer.writerow(row)

                                    # If tornado intensity selection is EF-4...
                                    elif self.stringTornadoIntensity.get() == \
                                                 "EF-4":

                                        # If tornado intensity in CSV is 4...
                                        if int(float(row[10])) == 4:

                                            # Write the row to the output CSV.
                                            self.csv_Writer.writerow(row)

                                    # If tornado intensity selection is EF-5...
                                    elif self.stringTornadoIntensity.get() == \
                                                 "EF-5":

                                        # If tornado intensity in CSV is 5...
                                        if int(float(row[10])) == 5:

                                            # Write the row to the output CSV.
                                            self.csv_Writer.writerow(row)

                                    # If tornado intensity selection is
                                    # Custom...
                                    elif self.stringTornadoIntensity.get() == \
                                            "Custom...":

                                        # If CSV row's tornado intensity is
                                        # between the Min and Max range...
                                        if int(float(row[10])) >= \
                                            int(self.textCustomIntensityFrom) \
                                            and int(float(row[10])) <= \
                                                int(self.textCustomIntensityTo):
    ...
    def func_Check_If_Empty_CSV(self, csvName):
        ...
        try:
            ...
            with open(csvName) as csvFile:
                ...
                # to a raw tornado count variable.
                self.raw_tornado_count = sum(1 for row in csvFile)

                # If the tornado count is less than 1...
                if self.raw_tornado_count <= 1:

                    # Display error message explaining that no tornado
                    ...
                    # Else if tornado records exist, but are in excess of
                    # 500,000 records...
                    if self.raw_tornado_count >= 500000:
                        ...
                        self.func_Scroll_setOutputText("BE ADVISED:\n" +
                                    "The CSV contains " +
                                    '{:,}'.format(self.raw_tornado_count) +
                                    " features.\n" + "All additional processes "
    ...
    def func_CreateFeatureClass_from_CSV(
        ...
        try:
            ...
            # class.
            self.featureClass_Tornado_Unclipped_Unprojected = \
            ...
            self.featureClass_Tornado_Unclipped_Projected = \
                self.featureClass_Tornado_Unclipped_Unprojected + \
                "_Projected"
            ...
            arcpy.Project_management(
                self.featureClass_Tornado_Unclipped_Unprojected,
                self.featureClass_Tornado_Unclipped_Projected, pcsReference)
            ...
            self.func_Scroll_setOutputText("Renaming feature class " +
                        self.featureClass_Tornado_Unclipped_Projected + " to " +
                        self.featureClass_Tornado_Unclipped_Unprojected + "...",
                        None)
    ...
    def func_RadioButton_Clipping_Rules(self):
        ...
        try:
            ...
            if self.radioButton_Selection == 1:
                ...
                # Then execute function to clip tornado features to the
                ...
                # Execute function to verify if clipped tornado feature class
    ...
    def func_FeatureClass_Clip(self, inputFC, clipFC, outputFC_Name):
        ...
        # tornado point feature class to the user-selected Census polygon
        ...
        try:
            ...
            if self.radioButton_Selection == 1:
                ...
                self.func_Scroll_setOutputText(
                    "Clipping tornado features to 50 states and DC...", None)
            ...
            # If the user selects State...
                ...
                # Display this particular message before executing the clip.
                self.func_Scroll_setOutputText(
                    "Clipping tornado features to state selection(" +
            ...
            # If the user selects County...
                ...
                # Display this particular message before executing the clip.
                self.func_Scroll_setOutputText(
                    "Clipping tornado features to " +
                    self.stringCounty_Name.get() + ", " +
                    self.stringState_Name.get() + "...", None)

            # Perform a clip of the tornado point features to the specified
            # polygon boundary.
            ...
            if self.radioButton_Selection == 1:
                ...
                self.func_Scroll_setOutputText("Tornado features have been "
    ...
    def func_Check_If_Empty_Output_FeatureClass(self,
        ...
        # tornado point feature class have features once clipped.
        ...
        # variable.
        self.tornadoCountResult = \
            arcpy.GetCount_management(clipped_FeatureClass_Points)

        # Store the integer version of the tornado count to variable.
        self.clipped_tornadoCount = \
            int(self.tornadoCountResult.getOutput(0))

        # If user selects USA...
        if self.radioButton_Selection == 1:

            # Display this particular message with tornado count value.
            self.func_Scroll_setOutputText(str(self.clipped_tornadoCount) +
                    " tornado features observed within the 50 states and DC.",
                    None)
        ...
        if self.radioButton_Selection == 2:
            ...
            # Display this particular message with tornado count value.
            self.func_Scroll_setOutputText(str(self.clipped_tornadoCount) +
                                        " tornado features observed within " +
                                        self.stringState_Name.get() + ".", None)
        ...
        # If zero tornado features present within the feature class...
        if self.clipped_tornadoCount < 1:
            # Display this message and error message to the user, since the
            # script can't proceed any farther.
            self.func_Scroll_setOutputText("1 or fewer tornado features "
                        "present within the timespan/intensity/location.\n" +
                        "Unable to continue with analysis.", color_Red)

            messagebox.showerror(errorMessage_Header,
                                 "No tornado features present within the "
    ...
    def func_Controls_For_Analysis_Options(self, featureClass_Mask):
        ...
        try:
            ...
            if self.statusAnalysis_OutputToCSVFile.get() == 1:
                ...
                if self.radioButton_Selection == 1:
                    ...
                    # Perform spatial join of clipped tornado feature class
                    ...
                    if self.stringTornadoTimespan.get() != "Custom..." and \
                        ...
                        csvFile.write("Timespan:," +
                                      self.stringTornadoTimespan.get() + ",\n")
                        csvFile.write("Size Range (EF):," +
                                      self.stringTornadoIntensity.get() + ",\n")
                    ...
                    csvFile.write("\n")
                    csvFile.write("USA (and DC) Tornadoes:," +
                                  str(self.clipped_tornadoCount) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("Nationwide Tornado Data,\n")
                    csvFile.write("V,V,V,\n")
                    csvFile.write("Minimum Size,Maximum Size,Mode Size,\n")
                    ...
                    with arcpy.da.SearchCursor("in_memory/SpatialJoin",
                        ...
                        for row in cursor:
                            ...
                            # Add the Intensity to the US list.
                            magList_US.append(row[1])

                    try:

                        # Calculate mode of all Intensities within US list.
                    ...
                    # Min size, max size, and mode size.
                    csvFile.write(str(min(magList_US)) + "," +
                            str(max(magList_US)) + "," +
                            modeString_US + ",\n")

                    csvFile.write("\n")
                    csvFile.write("Tornado Data per State,\n")
                    csvFile.write("V,V,V,V,V,V,\n")
                    csvFile.write("States with Tornadoes,Tornado Count,"
                                  "Minimum Size,Maximum Size,"
                                  "Mode Size,State FIPS Code,\n")

                    # Create empty list for storing tornado/intensity values.
                    magList = []

                    # For each state value in the ordered dictionary...
                    for key in dictKeys_OrderedDict_StateNames:

                        # Set counter to zero, for counting tornado within
                        ...
                        if counter > 0:
                            ...
                            # Write the following data to the data counts CSV.
                            # State name, how many tornadoes, min size, max
                            # size, mode size, and FIPs.
                ...
                if self.radioButton_Selection == 2:
                    ...
                    # Execute function to obtain total tornado count within
                    # the US.
                    ...
                    csvFile.write("\n")
                    csvFile.write("USA (and DC) Tornadoes:," +
                                  str(self.usa_tornado_count) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("State Tornadoes - " +
                                  ...
                                  ",\n")
                    csvFile.write("\n")
                    csvFile.write("Statewide Tornado Data,\n")
                    csvFile.write("V,V,V,\n")
                    csvFile.write("Minimum Size,Maximum Size,Mode Size,\n")
                    ...
                    with arcpy.da.SearchCursor("in_memory/SpatialJoin",
                        ...
                        for row in cursor:
                            ...
                            # Add the intensity to the state list.
                            magList_State.append(row[1])

                    try:

                        # Calculate mode of all intensities within state list.
                    ...
                    csvFile.write("\n")
                    csvFile.write("Tornado Data per County\n")
                    csvFile.write("V,V,V,V,V,\n")
                    csvFile.write("Counties with Tornadoes,Tornado Count,"
                                  "Minimum Size,Maximum Size,"
                                  "Mode Size,\n")
                    ...
                    for key in GUI_CountiesPerState.dict_state_counties[
                        ...
                        # Open the in_memory spatial join as a Search Cursor,
                        # with analysis fields set to county name and intensity.
                        with arcpy.da.SearchCursor("in_memory/SpatialJoin",
                                            [census_Shapefile_Field_CountyName,
                                            analysis_Mag_Field]) as cursor:
                        ...
                        if counter > 0:
                            ...
                            # Write the following data to the data counts CSV.
                            # County name, how many tornado, min size, max
                            # size, and mode size.
                            csvFile.write(str(key) + "," + str(counter) +
                ...
                if self.radioButton_Selection == 3:
                    ...
                    csvFile.write("State Tornadoes - " +
                                ...
                                str(dictKeys_OrderedDict_StateNames[
                                self.stringState_Name.get()]) + "):," +
                                str(self.state_tornado_count) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("County Tornadoes - " +
                                  self.stringCounty_Name.get() + ":," +
                                  str(self.clipped_tornadoCount) + ",\n")
                    csvFile.write("\n")
                    csvFile.write("County Tornado Data\n")
                    csvFile.write("V,V,V,\n")
                    csvFile.write("Minimum Size,Maximum Size,"
                                  "Mode Size,\n")
                    ...
                    for key in GUI_CountiesPerState.dict_state_counties[
                        ...
                        # Set counter to zero, for counting tornadoes within
        ...
        try:
            ...
            if self.statusAnalysis_NaturalNeighbor.get() == 1:
                ...
                # If the total number of tornado features reach 14,750,000...
                if self.raw_tornado_count >= 14750000:
                    ...
                    self.func_Scroll_setOutputText(
                        ...
                        "input tornado count nears 15,000,000.\n" +
        ...
        try:
            ...
            if self.statusAnalysis_OptHotSpot.get() == 1:
                ...
                # If the clipped tornado count is less than 30 features...
                if self.clipped_tornadoCount < 30:
                    ...
                    self.func_Scroll_setOutputText(
                        ...
                        " 30 tornado within the sampling area.\n" +
                        "Skipping this analysis...", color_Red)
                ...
                # If the clipped tornado count is less than 60 features...
                if self.clipped_tornadoCount < 60:
                    ...
                    self.func_Scroll_setOutputText(
                        ...
                        " ANALYSIS FIELD OPTION):\n" +
                        "According to Esri, this analysis requires a minimum of"
                        " 60 tornado within the sampling area.\n" +
                        "Skipping this analysis...", color_Red)
    ...
    def func_CSV_DataCount_Nationwide_Count(self):
        ...
        # This function controls the counting of all tornado within the US.
        ...
        try:
            ...
            # Create new feature class variable in_memory for clipped tornado
            # points.
            clipped_Output_FeatureClass = "in_memory/ClippedOutputFC_US"

            # Perform a clip of raw tornado points to the 50 US states and DC
            ...
            # Perform a count on the clipped tornado features found within
            # the in_memory clipped feature class.
            tornadoCountResult_US = arcpy.GetCount_management(
                clipped_Output_FeatureClass)

            # Store the integer tornado count as a variable for use within
            # other functions.
            self.usa_tornado_count = \
                int(tornadoCountResult_US.getOutput(0))
    ...
    def func_CSV_DataCount_Statewide_Count(self):
        ...
        # This function controls the counting of all tornado within the
        # user-selected state.
        ...
        try:
            ...
            # Perform a clip of raw tornado points to the user-selected state
            ...
            # Perform a count on the clipped tornado features found within
            # the in_memory clipped feature class.
            tornadoCountResult_State = arcpy.GetCount_management(
                clipped_Output_FeatureClass)

            # Store the integer tornado count as a variable for use within
            # the in_memory clipped feature class.
            self.state_tornado_count = \
                int(tornadoCountResult_State.getOutput(0))
    ...
    def func_windowResize(self):
        ...
        self.checkboxExpansionWidth = guiWindow_TornadoOptions_Width + 300

        # Set the GUI expansion height if timespan/intensity drop-downs show
        # "Custom...".
        self.customSelection_Height = guiWindow_TornadoOptions_Height + 75
        ...
        if self.statusVar_Checkbutton_Options.get() == 0 and \
                self.stringTornadoTimespan.get() != "Custom..." and \
                self.stringTornadoIntensity.get() != "Custom...":
        ...
        # or custom intensity are selected...
                ...
                (self.stringTornadoTimespan.get() == "Custom..." or
                 self.stringTornadoIntensity.get() == "Custom..."):
    ...
    def func_Enable_Buttons(self):
        ...
        self.combo_TornadoIntensity.config(state="readonly")
        self.combo_TornadoIntensity.update()

        # Enable Timespan drop-down list.
        self.combo_TornadoTimespan.config(state="readonly")
        self.combo_TornadoTimespan.update()
    ...
    def func_Disable_Buttons(self):
        ...
        self.combo_TornadoIntensity.config(state=tkinter.DISABLED)
        self.combo_TornadoIntensity.update()

        # Disable the Timespan drop-down list.
        self.combo_TornadoTimespan.config(state=tkinter.DISABLED)
        self.combo_TornadoTimespan.update()
